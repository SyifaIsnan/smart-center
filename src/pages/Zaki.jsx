...
import { getZakiDashboardData, getZakiHealth, getZakiNetwork, getZakiProcesses } from '../api/zakiApi';
...
export default function Zaki() {
  ...
  /* ── State untuk Chart History ── */
  const [cpuHist,  setCpuHist]  = useState(Array(20).fill({ t: '', val: 0 }));
  const [ramHist,  setRamHist]  = useState(Array(20).fill({ t: '', val: 0 }));
  const [diskHist, setDiskHist] = useState(Array(20).fill({ t: '', val: 0 }));

  /* ── Fetch Real API ── */
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [dataDashboard, dataHealth, dataNetwork, dataProcesses] = await Promise.all([
        ...
        setCpuHist(prev => [...prev.slice(1), { t: 'T', val: curCpu }]);
        setRamHist(prev => [...prev.slice(1), { t: 'T', val: curRam }]);
        setDiskHist(prev => [...prev.slice(1), { t: 'T', val: curDisk }]);
    ...
    <>
      ...
      <div className="page-content section-gap" style={{ maxWidth: '100%' }}>
          ...
          >
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px', flex: 1 }}>
              {[
                ...
                { label: 'CPU Cores', value: sysData?.cpu?.cores       ?? '—' },
                { label: 'Uptime',    value: uptime },
                { label: 'CPU Model', value: (sysData?.cpu?.model || '').split('@')[0].trim() || '—' },
        ...
        <div className="grid-3" style={{ alignItems: 'stretch' }}>
          ...
          <Card 
            title="CPU Usage" 
            subtitle={`${sysData?.cpu?.cores ?? '—'} Cores Active`} 
            rightElement={
          ...
          >
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <div className="server-details" style={{ marginTop: '20px' }}>
                {[
                  { label: 'User / Sys',  value: `${(sysData?.cpu?.usage?.user / 1000000).toFixed(1)}M / ${(sysData?.cpu?.usage?.system / 1000000).toFixed(1)}M` },
                  { label: 'Idle / Wait', value: `${(sysData?.cpu?.usage?.idle / 1000000).toFixed(1)}M / ${sysData?.cpu?.usage?.iowait || 0}` },
                  { label: 'Load 1m',     value: loadAvg?.['1min'] ?? '—' },
                  { label: 'Load 5m',     value: loadAvg?.['5min'] ?? '—' },
                ].map((r, i) => (
          ...
          <Card 
            title="RAM Usage" 
            subtitle={`Total ${sysData?.memory?.human?.total ?? '—'}`} 
            rightElement={
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <div className="server-details" style={{ marginTop: '20px' }}>
                {[
                  ...
                  { label: 'Tersedia',   value: sysData?.memory?.human?.available  ?? '—' },
                  { label: 'Cache',      value: ((sysData?.memory?.cache || 0) / (1024 * 1024)).toFixed(1) + ' MB' },
                  { label: 'Free',       value: sysData?.memory?.human?.free ?? '—' },
                ].map((r, i) => (
                  <div key={i} className="server-detail-row" style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px dashed #e2e8f0' }}>
                    <span className="server-detail-label" style={{ fontSize: 12, color: 'var(--text-muted)' }}>{r.label}</span>
                    <span className="server-detail-value" style={{ fontSize: 12, fontWeight: 500 }}>{r.value}</span>
          ...
          <Card 
            title="Disk Usage" 
            subtitle={`Total ${sysData?.disk?.human?.total ?? '—'}`} 
            rightElement={
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <div className="server-details" style={{ marginTop: '20px' }}>
                {[
                  ...
                  { label: 'Tersedia', value: sysData?.disk?.human?.available  ?? '—' },
        ...
        {/* ══ ROW 2: Network Interfaces + Docker ══ */}
        <div className="grid-2" style={{ marginTop: 20, alignItems: 'stretch' }}>
          ...
          <Card 
            ...
            style={{ height: '100%', display: 'flex', flexDirection: 'column' }}
          >
            <div style={{ marginTop: 10, flex: 1, maxHeight: 260, overflowY: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['INTERFACE', 'IP ADDRESS', 'MAC', 'TIPE'].map(h => (
                      <th key={h} style={{ padding: '8px 8px', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textAlign: 'left', position: 'sticky', top: 0, background: '#fff', zIndex: 10, borderBottom: '1px solid var(--card-border)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {network.map((n, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '10px 8px' }}>
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4, background: n.internal ? 'rgba(100,116,139,0.1)' : 'rgba(34,197,94,0.1)', color: n.internal ? '#64748b' : '#22c55e' }}>
          ...
          <Card 
            ...
            rightElement={<Badge variant="success" dot>{sysData?.docker?.running ?? 0} Running</Badge>} 
            delay="0.36s" 
            style={{ height: '100%', display: 'flex', flexDirection: 'column' }}
          >
            <div style={{ maxHeight: 260, overflowY: 'auto', marginTop: 10, flex: 1 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['CONTAINER', 'IMAGE', 'STATUS', 'CPU%'].map(h => (
                      <th key={h} style={{ padding: '8px 4px', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textAlign: 'left', position: 'sticky', top: 0, background: '#fff', zIndex: 10, borderBottom: '1px solid var(--card-border)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(sysData?.docker?.containers ?? []).map((c, i) => {
                     ...
                     const cName = c.name.length > 20 ? c.name.substring(0,20)+'...' : c.name;
                     const cImg = c.image.split('/').pop();
                     
                     return (
                      <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        <td style={{ padding: '10px 4px' }} title={c.name}>
                          <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 12, fontWeight: 600, color: '#3b82f6' }}>{cName}</div>
                        </td>
                        <td style={{ padding: '10px 4px', fontFamily: 'monospace', fontSize: 11, color: 'var(--text-muted)' }} title={c.image}>{cImg}</td>
                        <td style={{ padding: '10px 4px' }}>
                          <span style={{ fontSize: 11, color: (c.status ?? '').toLowerCase().includes('up') ? '#22c55e' : '#ef4444' }}>{c.status}</span>
                        </td>
                        <td style={{ padding: '10px 4px', fontFamily: 'monospace', fontSize: 12, fontWeight: 600, color: cCpu > 10 ? '#f59e0b' : 'var(--foreground)' }}>
                          {c.stats?.cpu || '0%'}
                        </td>
                      </tr>
                    );
